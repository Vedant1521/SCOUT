...
public:
    ...
    std::experimental::optional<T> pop(int timeout_ms = 100) {
        std::unique_lock<std::mutex> lock(mutex_);
...
class Rules {
public:
    // Stream for info messages: cout normally, cerr in JSON mode
    std::ostream* info_stream = &std::cout;
    
    void blockIP(const std::string& ip) {
        std::lock_guard<std::mutex> lock(mutex_);
        blocked_ips_.insert(parseIP(ip));
        blocked_ip_strings_.push_back(ip);
        *info_stream << "[Rules] Blocked IP: " << ip << "\n";
    }
    ...
    void blockApp(const std::string& app) {
        ...
        for (int i = 0; i < static_cast<int>(AppType::APP_COUNT); i++) {
            if (appTypeToString(static_cast<AppType>(i)) == app) {
                ...
                blocked_app_strings_.push_back(app);
                *info_stream << "[Rules] Blocked app: " << app << "\n";
                return;
            }
        }
    ...
    void blockDomain(const std::string& domain) {
        ...
        *info_stream << "[Rules] Blocked domain: " << domain << "\n";
    }
    
    void blockPort(uint16_t port) {
        std::lock_guard<std::mutex> lock(mutex_);
        blocked_ports_.insert(port);
        *info_stream << "[Rules] Blocked port: " << port << "\n";
    }
    ...
    // Accessors for rule strings (used by JSON output)
    const std::vector<std::string>& getBlockedIPStrings() const { return blocked_ip_strings_; }
    const std::vector<std::string>& getBlockedAppStrings() const { return blocked_app_strings_; }
    const std::vector<std::string>& getBlockedDomainStrings() const { return blocked_domains_; }
...
private:
    ...
    std::vector<std::string> blocked_ip_strings_;
    std::vector<std::string> blocked_app_strings_;
...
struct Stats {
    ...
    // Blocked connection details (for JSON output)
    struct BlockedEntry {
        std::string src_ip;
        std::string dst_ip;
        std::string app;
    ...
    std::vector<BlockedEntry> blocked_entries;
    ...
    void recordBlocked(const std::string& src, const std::string& dst,
                       const std::string& app, const std::string& domain) {
        std::lock_guard<std::mutex> lock(blocked_mutex);
        blocked_entries.push_back({src, dst, app, domain});
    }
};
    ...
    void run() {
        while (running_) {
            ...
            if (!flow.blocked) {
                ...
                if (flow.blocked) {
                    // Record blocked connection details for JSON output
                    ...
                    stats_->recordBlocked(
                        formatIP(pkt.tuple.src_ip),
                        formatIP(pkt.tuple.dst_ip),
...
public:
    ...
    DPIEngine(const Config& cfg) : config_(cfg), json_output_(cfg.json_output) {
        int total_fps = cfg.num_lbs * cfg.fps_per_lb;
        
        // In JSON mode, redirect info messages to stderr to keep stdout clean
        if (json_output_) {
            rules_.info_stream = &std::cerr;
        }
        
        std::ostream& out = json_output_ ? std::cerr : std::cout;
        out << "\n";
        out << "╔══════════════════════════════════════════════════════════════╗\n";
        out << "║              DPI ENGINE v2.0 (Multi-threaded)                 ║\n";
        out << "╠══════════════════════════════════════════════════════════════╣\n";
        out << "║ Load Balancers: " << std::setw(2) << cfg.num_lbs 
    ...
    bool process(const std::string& input_file, const std::string& output_file) {
        ...
        (json_output_ ? std::cerr : std::cout) << "[Reader] Processing packets...\n";
        RawPacket raw;
        ParsedPacket parsed;
        uint32_t pkt_id = 0;
        ...
        (json_output_ ? std::cerr : std::cout) << "[Reader] Done reading " << pkt_id << " packets\n";
        reader.close();
        ...
        // Print report
        if (json_output_) {
            printJSON(input_file, output_file, duration_ms);
        } else {
            printReport();
        }
...
private:
    ...
    void printReport() {
        ...
        if (!stats_.detected_snis.empty()) {
            ...
            for (const auto& entry : stats_.detected_snis) {
                std::cout << "  - " << entry.first << " -> " << appTypeToString(entry.second) << "\n";
            }
        }
    }
    ...
    void printJSON(const std::string& input_file, const std::string& output_file, int64_t duration_ms) {
        std::lock_guard<std::mutex> lock(stats_.app_mutex);
...
#endif
        char time_str[32];
        strftime(time_str, sizeof(time_str), "%Y-%m-%dT%H:%M:%SZ", &tm_buf);
        
        uint64_t total = stats_.total_packets.load();
        
        // Sort app breakdown by count descending
        std::vector<std::pair<AppType, uint64_t>> sorted_apps(
            stats_.app_counts.begin(), stats_.app_counts.end());
        std::sort(sorted_apps.begin(), sorted_apps.end(),
                  [](const auto& a, const auto& b) { return a.second > b.second; });
        ...
        json << "{\n";
        json << "  \"engine_version\": \"2.0\",\n";
        json << "  \"timestamp\": \"" << time_str << "\",\n";
        json << "  \"input_file\": \"" << escapeJSON(input_file) << "\",\n";
        json << "  \"output_file\": \"" << escapeJSON(output_file) << "\",\n";
        
        // Summary
        json << "  \"summary\": {\n";
        json << "    \"total_packets\": " << total << ",\n";
        json << "    \"forwarded\": " << stats_.forwarded.load() << ",\n";
        json << "    \"dropped\": " << stats_.dropped.load() << ",\n";
        json << "    \"total_bytes\": " << stats_.total_bytes.load() << ",\n";
        json << "    \"duration_ms\": " << duration_ms << "\n";
        json << "  },\n";
        
        // Protocol breakdown
        json << "  \"protocol_breakdown\": {\n";
        json << "    \"tcp\": " << stats_.tcp_packets.load() << ",\n";
        json << "    \"udp\": " << stats_.udp_packets.load() << ",\n";
        uint64_t other = total > (stats_.tcp_packets.load() + stats_.udp_packets.load())
                       ? total - stats_.tcp_packets.load() - stats_.udp_packets.load() : 0;
        json << "    \"other\": " << other << "\n";
        json << "  },\n";
        
        // Application breakdown
        json << "  \"application_breakdown\": [\n";
        for (size_t i = 0; i < sorted_apps.size(); i++) {
            const auto& entry = sorted_apps[i];
            double pct = total > 0 ? (100.0 * entry.second / total) : 0;
            json << "    {\"app\": \"" << escapeJSON(appTypeToString(entry.first))
                 << "\", \"packets\": " << entry.second
                 << ", \"percentage\": " << pct << "}";
        ...
        json << "  \"detected_domains\": [\n";
        {
            size_t idx = 0;
            for (const auto& entry : stats_.detected_snis) {
                json << "    {\"domain\": \"" << escapeJSON(entry.first)
                     << "\", \"app\": \"" << escapeJSON(appTypeToString(entry.second)) << "\"}";
                if (idx + 1 < stats_.detected_snis.size()) json << ",";
                json << "\n";
                idx++;
            }
        }
        ...
        // Blocked connections
        json << "  \"blocked_connections\": [\n";
        {
            std::lock_guard<std::mutex> block(stats_.blocked_mutex);
            for (size_t i = 0; i < stats_.blocked_entries.size(); i++) {
                const auto& e = stats_.blocked_entries[i];
                json << "    {\"src_ip\": \"" << escapeJSON(e.src_ip)
                     << "\", \"dst_ip\": \"" << escapeJSON(e.dst_ip)
                     << "\", \"app\": \"" << escapeJSON(e.app)
                     << "\", \"domain\": \"" << escapeJSON(e.domain) << "\"}";
                if (i + 1 < stats_.blocked_entries.size()) json << ",";
                json << "\n";
            }
        }
        json << "  ],\n";
        ...
        json << "    \"blocked_ips\": [";
        {
            const auto& ips = rules_.getBlockedIPStrings();
        ...
        json << "    \"blocked_apps\": [";
        {
            const auto& apps = rules_.getBlockedAppStrings();
            for (size_t i = 0; i < apps.size(); i++) {
                json << "\"" << escapeJSON(apps[i]) << "\"";
                if (i + 1 < apps.size()) json << ", ";
            }
        }
        json << "],\n";
        // Blocked domains
        ...
        {
            const auto& doms = rules_.getBlockedDomainStrings();
            for (size_t i = 0; i < doms.size(); i++) {
                json << "\"" << escapeJSON(doms[i]) << "\"";
                if (i + 1 < doms.size()) json << ", ";
            }
        }
        json << "],\n";
        json << "    \"blocked_ports\": [";
        {
            auto ports = rules_.getBlockedPorts();
            for (size_t i = 0; i < ports.size(); i++) {
                json << ports[i];
                if (i + 1 < ports.size()) json << ", ";
            }
        }
        json << "]\n";
        json << "  },\n";
        ...
        json << "    \"load_balancers\": [\n";
        for (size_t i = 0; i < lbs_.size(); i++) {
            json << "      {\"id\": " << i << ", \"dispatched\": " << lbs_[i]->dispatched() << "}";
            if (i + 1 < lbs_.size()) json << ",";
            json << "\n";
        }
        json << "    ],\n";
        json << "    \"fast_paths\": [\n";
        for (size_t i = 0; i < fps_.size(); i++) {
            json << "      {\"id\": " << i << ", \"processed\": " << fps_[i]->processed() << "}";
            if (i + 1 < fps_.size()) json << ",";
            json << "\n";
        }
        json << "    ]\n";
        json << "  }\n";
        ...
        // Write the complete JSON to stdout (the ONLY thing on stdout in JSON mode)
...
Options:
  ...
  --block-port <port>    Block destination port (e.g., 443)
  --lbs <n>              Number of load balancer threads (default: 2)
  --fps <n>              FP threads per LB (default: 2)
  --json                 Output results as JSON to stdout (for web integration)

Example:
  )" << prog << R"( capture.pcap filtered.pcap --block-app YouTube --block-ip 192.168.1.50
  )" << prog << R"( capture.pcap filtered.pcap --block-app YouTube --json
)";
}
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 3; i < argc; i++) {
        ...
        else if (arg == "--block-port" && i + 1 < argc) block_ports.push_back(static_cast<uint16_t>(std::stoi(argv[++i])));
    ...
    for (const auto& port : block_ports) engine.blockPort(port);
    
    if (!engine.process(input, output)) {
        return 1;
    }
    
    (cfg.json_output ? std::cerr : std::cout) << "\nOutput written to: " << output << "\n";
    return 0;
}
